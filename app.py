...
PROFILE_PRESETS = {
    "Balanced": {
        "weights": {"nw": 0.12, "legacy": 0.12, "trad": 0.22, "stability": 0.30, "risk": 0.10, "drag": 0.14, "trad_share": 0.16},
    ...
    "Tax-Efficient Stability": {
        "weights": {"nw": 0.02, "legacy": 0.14, "trad": 0.36, "stability": 0.12, "risk": 0.06, "drag": 0.42, "trad_share": 0.28},
    ...
    "Legacy Focused": {
        "weights": {"nw": 0.01, "legacy": 0.72, "trad": 0.30, "stability": 0.05, "risk": 0.03, "drag": 0.10, "trad_share": 0.34},
    ...
    "Spend With Confidence": {
        "weights": {"nw": 0.02, "legacy": 0.02, "trad": 0.08, "stability": 0.64, "risk": 0.18, "drag": 0.06, "trad_share": 0.08},
...
def build_strategy_selection_summary(profile_name: str, preferences: dict) -> dict:
    ...
    if preferences.get("minimize_trad_ira_for_heirs"):
        ...
        nw_component = 0.03 * weights["nw"] * nw_signal
        stability_component = weights["stability"] * stability_signal
        trad_component = 1.35 * weights["trad"] * (max(trad_penalty_base, trad_at_rmd_ratio) ** 1.70)
        trad_share_component = 1.20 * weights.get("trad_share", 0.0) * (trad_share_penalty_base ** 2.10)
        drag_component = 0.95 * weights.get("drag", 0.0) * (drag_penalty_base ** 1.18)
        heir_tax_component = 1.35 * weights["trad"] * (heir_tax_penalty_base ** 1.75)
        risk_component = 0.80 * weights["risk"] * (risk_penalty_base ** 1.05)
    elif profile_name == "Spend With Confidence":
        legacy_component = 0.60 * weights["legacy"] * base_legacy_signal
        nw_component = 0.30 * weights["nw"] * (nw_signal ** 0.78)
        stability_component = weights["stability"] * (
            0.20 * math.log1p(max(0.0, stability_support_ratio))
            + 0.24 * math.log1p(max(0.0, income_coverage_ratio))
            + 0.26 * math.log1p(max(0.0, survivor_coverage_ratio))
            + 0.20 * (ss_value_signal ** 1.06)
            + 0.10 * (early_income_ratio ** 1.25)
        )
        trad_component = 1.05 * weights["trad"] * (trad_penalty_base ** 1.16)
        trad_share_component = 1.08 * weights.get("trad_share", 0.0) * (trad_share_penalty_base ** 1.18)
        drag_component = 0.85 * weights.get("drag", 0.0) * (drag_penalty_base ** 1.02)
        heir_tax_component = 0.0
        risk_component = weights["risk"] * (
            0.70 * (risk_penalty_base ** 1.12)
            + 0.30 * (delay_extreme_ratio ** 1.20)
        )
    elif profile_name == "Tax-Efficient Stability":
        legacy_component = weights["legacy"] * (base_legacy_signal ** 1.00)
        nw_component = 0.35 * weights["nw"] * (nw_signal ** 0.76)
        stability_component = weights["stability"] * (
            0.36 * math.log1p(max(0.0, stability_support_ratio))
            + 0.20 * math.log1p(max(0.0, income_coverage_ratio))
            + 0.12 * math.log1p(max(0.0, survivor_coverage_ratio))
            + 0.10 * (ss_value_signal ** 0.92)
        )
        trad_component = 1.35 * weights["trad"] * (max(trad_penalty_base, trad_at_rmd_ratio) ** 1.75)
        trad_share_component = 1.20 * weights.get("trad_share", 0.0) * (trad_share_penalty_base ** 1.85)
        drag_component = 1.55 * weights.get("drag", 0.0) * (
            (drag_penalty_base ** 1.35)
            + 0.55 * (magi_excess_ratio ** 1.65)
            + 0.35 * (irmaa_hit_years_ratio ** 1.45)
        )
        heir_tax_component = 0.0
        risk_component = weights["risk"] * (
            0.55 * (risk_penalty_base ** 1.10)
            + 0.45 * (magi_excess_ratio ** 1.55)
        )
    elif profile_name == "Balanced":
        legacy_component = weights["legacy"] * (base_legacy_signal ** 1.01)
        nw_component = 0.58 * weights["nw"] * (nw_signal ** 0.84)
        stability_component = 1.20 * weights["stability"] * (
            0.38 * math.log1p(max(0.0, stability_support_ratio))
            + 0.22 * math.log1p(max(0.0, income_coverage_ratio))
            + 0.18 * math.log1p(max(0.0, survivor_coverage_ratio))
            + 0.10 * (ss_value_signal ** 0.98)
            + 0.12 * (early_income_ratio ** 1.10)
        )
        trad_component = 1.12 * weights["trad"] * (trad_penalty_base ** 1.42)
        trad_share_component = 1.10 * weights.get("trad_share", 0.0) * (trad_share_penalty_base ** 1.65)
        drag_component = 1.10 * weights.get("drag", 0.0) * (
            (drag_penalty_base ** 1.18)
            + 0.20 * (magi_excess_ratio ** 1.35)
        )
        heir_tax_component = 0.0
        risk_component = weights["risk"] * (
            0.55 * (risk_penalty_base ** 1.08)
            + 0.45 * (delay_extreme_ratio ** 1.25)
        )
    else:
        legacy_component = weights["legacy"] * (base_legacy_signal ** 1.02)
